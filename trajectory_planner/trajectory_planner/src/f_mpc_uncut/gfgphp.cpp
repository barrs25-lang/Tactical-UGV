...
// Input: ugv, mpc_rules, bag_of_many_things: structures containing system and problem parameters; see <structures.h> for details
...
void gfgphp(struct System* ugv, struct MPC_Params* mpc_rules, Eigen::MatrixXf* X, Eigen::MatrixXf* U, struct Matrix_Set* BoMT)
{
	...
	// Compute Fx * X
	(*BoMT).FxX.noalias() = (*ugv).eig_Fx_hard * (*X);

	// Iterate over the number of time steps
	for (unsigned short int i = 0; i < (*mpc_rules).T; i++)
	{
		(*BoMT).dx.col(i) = ((-(*BoMT).FxX).col(i) + (*ugv).eig_x_hard_bounds.col(i)).cwiseInverse();
	}

	// Compute Fu * U
	(*BoMT).FuU.noalias() = (*ugv).eig_Fu_hard * (*U);

	// Compute du[i] = 1 / (fu[i%m] - FuU[i])
	(*BoMT).du.noalias() = ((-(*BoMT).FuU).colwise() + (*ugv).eig_u_hard_bounds).cwiseInverse();
	...
	(*BoMT).FxTdx.noalias() = (*ugv).eig_Fx_hard.transpose() * (*BoMT).dx;

	// Compute Ptdu = Fu^T du
	(*BoMT).FuTdu.noalias() = (*ugv).eig_Fu_hard.transpose() * (*BoMT).du;
...
void dtildhat(struct System* ugv, struct MPC_Params* mpc_rules, Eigen::MatrixXf* X, Eigen::MatrixXf* U, struct Matrix_Set* BoMT)
{

	// Compute Fz
	(*BoMT).FxtildeX.noalias() = (*ugv).eig_Fx_soft * (*X);
	(*BoMT).FutildeU.noalias() = (*ugv).eig_Fu_soft * (*U);

	// Declare float to store computation of exponential
	float e_i;

	// Float arrays to store the d values
	float* dtilde = new float[(*ugv).num_soft_x* (*mpc_rules).T]; 
	float* dhat = new float[(*ugv).num_soft_x* (*mpc_rules).T]; 
	...
	if ((*mpc_rules).centering_soft_constraints)
	{
		...
		{
			Px_kernel = (*ugv).eig_Fx_soft.transpose().fullPivLu().kernel();

			for (int i = 0; i < (*ugv).num_soft_x; i++)
			{
				for (int j = 0; j < 10; j++)
				{
					if (Px_kernel.col(i).lpNorm<Eigen::Infinity>() < 1)
					{
						d_0 = Eigen::MatrixXf::Zero((*ugv).num_soft_x,1);
						d_0.col(0) = Px_kernel.col(i);
						trying = false;
						for (int k = 0; k < (*ugv).num_soft_x; k++)
						{
			...
			if (success)
			{
				...
				for (int i = 0; i < (*ugv).num_soft_x; i++)
				{
					// (*BoMT).rho_i_x(i,0) = ( 1 / (*ugv).eig_x_soft_bounds[i] ) * log((1/d_0(i,0)) - 1);
					(*BoMT).rho_i_x(i,0) = ( 1 / (*ugv).eig_x_soft_bounds(i,0) ) * log((1/d_0(i,0)) - 1);
	...
	for(unsigned short int i = 1; i < (*mpc_rules).T+1; ++i) //check optimality w.r.t. row-major/col-major order
	{
		...
		for(unsigned short int j = (i-1)*(*ugv).num_soft_x; j < (i)*(*ugv).num_soft_x; ++j)
		{

			// Compute the exponential
			e_i = exp((*mpc_rules).mu_13 * ((*BoMT).FxtildeX.data()[j] - (*ugv).eig_x_soft_bounds.data()[j%(*ugv).num_soft_x])); //corresponds to e_i+ in eq19 of Richards
	...
	(*BoMT).d_tilde_x = Eigen::Map<Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic>>(dtilde, (*ugv).num_soft_x, (*mpc_rules).T);
	(*BoMT).d_hat_x = Eigen::Map<Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic>>(dhat, (*ugv).num_soft_x, (*mpc_rules).T);

	// New float arrays to store d for control constraints
	dtilde = new float[(*ugv).num_soft_u* (*mpc_rules).T]; 
	dhat = new float[(*ugv).num_soft_u* (*mpc_rules).T]; 
	...
	for(unsigned short int i = 0; i < (*mpc_rules).T; ++i)
	{
		...
		for(int j = 0; j < (*ugv).num_soft_u; ++j)
		{

			// Compute the exponential
			e_i = exp( (*mpc_rules).mu_13 * ((*BoMT).FutildeU(j,i) - (*ugv).eig_u_soft_bounds(j) ) ); //corresponds to e_i+ in eq19 of Richards
			...
			if(!isinf(e_i))
			{
				...
				// Compute dtilde and dhat
				dtilde[j+i*(*ugv).num_soft_u] = (e_i<1)?(e_i/(1.0f + e_i)):(1.0f/(1.0f + 1.0f/e_i)); //evaluates eq22 of richards using the smaller of e_i+ and e_i- for control segment
				dhat[j+i*(*ugv).num_soft_u] = e_i / ((1.0f + e_i) * (1.0f + e_i)); //evaluates eq24 of Richards for control segment
			...
			else 
			{
				...
				// Compute dtilde and dhat
				dtilde[j+i*(*ugv).num_soft_u] = 1.0f;
				dhat[j+i*(*ugv).num_soft_u] = 0.0f;
	...
	(*BoMT).d_tilde_u = Eigen::Map<Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic>>(dtilde, (*ugv).num_soft_u, (*mpc_rules).T);
	(*BoMT).d_hat_u = Eigen::Map<Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic>>(dhat, (*ugv).num_soft_u, (*mpc_rules).T);
