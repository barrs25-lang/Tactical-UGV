...
struct System {
	...
	float length; 							// Length of vehicle [meters]
	float width;							// Width of vehicle [meters]
	float height;							// height of vehicle [meters]
	float mass;								// Mass of vehicle [kg]

	// Physical properties of UGV bicycle model, used by the F_MPC_UNCUT feedback linearization law (see ugv_feedback_linearization_state_space.md)
	float cornering_stiffness_front;		// Front tire cornering stiffness P_Cf [N/rad]
	float cornering_stiffness_rear;		// Rear tire cornering stiffness P_Cr [N/rad]
	float dist_cg_to_front_axle;			// Distance from CG to front axle, a [m]
	float dist_cg_to_rear_axle;			// Distance from CG to rear axle, b [m]
	float vx_regularization_eps;			// Regularization epsilon preventing division by ~0 longitudinal velocity in the tire slip-angle computation
	float Fx_max;							// Maximum longitudinal force F_x [N], used in the feedback-linearized control law's barrier constraint
	float delta_f_max;						// Maximum front steering angle delta_f [rad], used in the feedback-linearized control law's barrier constraint

	// Hard constraint matrices and parameters
	int num_hard_x = 32; 					// Number of hard state constraints
	Eigen::MatrixXf eig_Fx_hard;			// F matrix for hard constraints on position/velocity and collision avoidance (dynamic x n) (think of each row in the matrix being the normal to a plane in the state-space. All states must not "cross" the plane)
	Eigen::MatrixXf eig_x_hard_bounds;		// f vector for hard constraints on position/velocity and collision avoidance (think of this as the offset for the plane defined by F. Really F and f form a plane equation F{1,1}x + F_{1,2}y + F_{1,3}z + ... = f_1)
	...
	Eigen::MatrixXf eig_Fx_soft; 			// F matrix for soft constraints on position/velocity and collision avoidance (dynamic x n) (think of each row in the matrix being the normal to a plane in the state-space. All states must not "cross" the plane)
	Eigen::MatrixXf eig_x_soft_bounds;		// f vector for soft constraints on position/velocity and collision avoidance (think of this as the offset for the plane defined by F. Really F and f form a plane equation F{1,1}x + F_{1,2}y + F_{1,3}z + ... = f_1)
	...
	Eigen::VectorXf X0;						// State vector chi = [x, y, xdot, ydot] (n X 1)
	Eigen::VectorXf prevX0;						// State vector from the previous planning cycle (n X 1)
