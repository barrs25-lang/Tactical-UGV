...
void F_MPC_UNCUT::communication_thread()
{
	...
	// 13 sockets: the original 12 plus index 12 "control_to_server", which sends the planned
	// control sequence (F_x, delta_f) -- see the CONTROL COMMUNICATIONS block below.
	CLIENT client("Parameter_Files/socket_parameters_trajectory_planner.txt", &time_to_exit, 13);
	...
	while(file_line.at(0) == '/' && file_line.at(1) == '/');
	ss >> control_freq;
	cout << "control_freq: " << control_freq << endl;
	...
	float control_time_us = 1000000/control_freq; // Microseconds
	...
	auto current_time_control = std::chrono::high_resolution_clock::now();
	...
	std::chrono::duration<double> timeElapsed_control = end_time - current_time_control;
	...
	// Control-sequence (F_x, delta_f) communication vars -- mirrors trajectory_send but
	// carries full_policy (eig_U) instead of full_trajectory (eig_X)
	string control_send;
	char* control_send_buffer = new char[6000];
	char* cstr_control = new char[6000];
	...
	while(!time_to_exit)
	{
		...
		if (std::chrono::duration_cast<std::chrono::microseconds>(timeElapsed_control).count() >= control_time_us)
		{

			// SEND THE CONTROL SEQUENCE (F_x, delta_f per horizon step, per planned segment)
			if (client.socket_active[12])
			{

				control_send.clear();
				control_send.append("U");
				control_send.append(",");
				while(pthread_mutex_trylock(&trajectory_lock))
				{
					usleep(1);
				...
				pthread_mutex_lock(&trajectory_lock);
					for (int j = 0; j < mpc_params.nu_X; j++)
					{
						for (int i = 0; i < mpc_params.T; i++)
						{
							for (int k = 0; k < quadrotor.m; k++)
							{

								float temp = full_policy[j](k,i);
								control_send.append( boost::lexical_cast<string>(temp) );
								if (j == mpc_params.nu_X-1 && i == mpc_params.T-1 && k == quadrotor.m-1)
								{
									control_send.append("!");
								}
								else
				...
				// Copy the string to a char buffer
				strcpy(cstr_control, control_send.c_str());
				memcpy(control_send_buffer, cstr_control, strlen(cstr_control)+1);

				// Send the control-sequence char buffer
				client.sockets[12]->process_sending(control_send_buffer,6000*sizeof(char));

			}

			current_time_control = std::chrono::high_resolution_clock::now();
		...
		timeElapsed_control = end_time - current_time_control;
