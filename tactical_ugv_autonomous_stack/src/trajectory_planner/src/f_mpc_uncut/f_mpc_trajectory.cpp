...
void F_MPC_UNCUT::find_closest_obstacle(Eigen::MatrixXf* temp_goal_pos, int segment_number)
{
	...
	direction(0,0) = cos(pose[14]);
	direction(1,0) = sin(pose[14]);
...
void F_MPC_UNCUT::update_quadrotor_pose()
{
	...
	// needed (e.g. in compute_force_steering_for_lambda_k_and_g_barrier) rather than stored in X0.
	current_position(0) = quadrotor.X0(0); // x
	current_position(1) = quadrotor.X0(1); // y
	current_position(2) = -pose[2];        // z
...
void F_MPC_UNCUT::find_closest_waypoint(float* xdiff, float* ydiff, float* zdiff, int* closest_traj_iterator)
{
	...
	for(int i = 0; i < localPath.rows()-1; ++i)
	{
		...
		(*zdiff) = current_position(2) - localPath(i,2);
...
void F_MPC_UNCUT::find_new_waypoint(Eigen::MatrixXf* goal, float* xdiff, float* ydiff, float* zdiff)
{
	...
	for(; local_traj_iterator < localPath.rows() - 1; local_traj_iterator++)
	{
		...
		if ( norm_diff > mpc_params.goal_tolerance )
		{
			...
			z = floor(current_position(2)*5.0 + 0.5);
	...
	pthread_mutex_lock(&constraint_lock);
		...
		for (int j = 0; j < collisionConstraints.rows(); j++)
		{
			...
			p_temp = current_position + n2*plane_size + n1*plane_size + n0*d;
		...
		for (int j = 0; j < collisionConstraints.rows(); j++)
		{
			...
			v = current_position - Eigen::Map<Eigen::Vector3f>(p.col(j).data(),3);
			...
			proj_quad_on_plane_temp = current_position + dist*n0;
		...
		while(not_in_constraint_set)
		{
			...
			proj_goal = lam*goal_temp+(1-lam)*current_position;
...
void F_MPC_UNCUT::get_trajectory_goal()
{
	...
	if (localPath.rows() >= 1)
	{
		...
		// 	(heading extraction removed with the quadrotor attitude-kinematics code)
	...
	while(!time_to_exit)
	{
		...
		pthread_mutex_lock(&pose_lock);
			...
			if (firstPassComplete)
			{
				...
				while ( 1 )
				{
					...
					if ( (prev_position - current_position).norm() > 0.6 )
					{
		...
		pthread_mutex_lock(&path_lock);
			...
			else
			{
				...
				localPath(0,2) = current_position(2);
			}
		...
		bomt.X_goal(0,0) = goal(0,0); // x
		bomt.X_goal(1,0) = goal(1,0); // y
		bomt.X_goal(2,0) = 0.0;       // target zero xdot at the goal (chi has no heading/z state)
		bomt.X_goal(3,0) = 0.0;       // target zero ydot at the goal
		...
		// if ((Eigen::Map<Eigen::MatrixXf>(current_position.data(),1,3) ).norm() < 0.6)
		// if ( (Eigen::Map<Eigen::MatrixXf>(current_position.data(),1,3) - localPath.row(localPath.rows()-1)).norm() < 0.6)
		// {
		...
		pthread_mutex_lock(&trajectory_lock);
			...
			while ( ii < mpc_params.nu_sk )
			{
				...
				if (ii == 0)
				{
					...
					for( int i = 0; i < mpc_params.T; i++ )
					{
						...
						eig_X.block(2,i,2,1)*=0;
						eig_U.col(i) = Eigen::MatrixXf::Zero(quadrotor.m,1);
					}
					...
					if (!firstSuccess)
					{
						...
						{
							g_barrier(j,0) = quadrotor.Fx_max*quadrotor.Fx_max*quadrotor.delta_f_max*quadrotor.delta_f_max;
						}
					...
					if (( quadrotor.X0(0) - goal(0,0) )*( quadrotor.X0(0) - goal(0,0) ) + ( quadrotor.X0(1) - goal(1,0) )*( quadrotor.X0(1) - goal(1,0) ) + ( current_position(2) - goal(2,0) )*( current_position(2) - goal(2,0) ) > 2.0)
					{				
					...
					// cout << "position: " << quadrotor.X0(0) << ", " << quadrotor.X0(1)  << ", " << current_position(2) << " goal: " << goal.transpose() << endl;
					...
					if (intermediate_success)
					{
						...
						// posterior computation of F_x, delta_f for a particular control policy
						// and evaluate g_barrier

						g_barrier_violation = compute_force_steering_for_lambda_k_and_g_barrier(&eig_X, &eig_U);

						if ((eig_X.block(0,mpc_params.T-1,2,1) - goal.block(0,0,2,1)).norm() > 2.0)
						{
				...
				else
				{
					...
					if (trajectory_plan_success)
					{
						for (int j = 0; j < mpc_params.T; j++)
						{
							eig_X.col(j) = prev_eig_X.col(mpc_params.T-1);
							eig_U.col(j) = prev_eig_U.col(mpc_params.T-1);
						}
						eig_X.block(2,0,2,mpc_params.T) *= 0;
					}
					...
					if (trajectory_plan_success)
					{
						...
						else
						{
							...
							while ( !( ( localPath.block(traj_iterator+ii+iii,0,1,2).transpose() - eig_X.block(0,mpc_params.T-1,2,1) ).norm() > 0.4 ) )
							{
								iii++;
					...
					// cout << eig_U << endl;
					// cout << "traj_iterator: " << traj_iterator <<endl;
					// cout << "start: " << current_position.transpose() << endl;					
					// cout << "goal: " << goal << endl;
					// cout << "robs: " << r_obs.transpose() << endl;
					// cout << "eig_Fx_hard: " << endl << quadrotor.eig_Fx_hard << endl;
					// cout << "eig_x_hard_bounds: " << endl << quadrotor.eig_x_hard_bounds << endl;
				...
				if (trajectory_plan_success)
				{
					...
					if ((eig_X.block(0,mpc_params.T-1,2,1) - goal.block(0,0,2,1)).norm() > 1.0)
					{
					...
					{	
						...
						if (ii == 0)
						{
							...
							for (int j = 0; j < mpc_params.T; j++)
							{
								...
								eig_X.block(2,0,2,j)*=0;
								eig_U.col(j) = Eigen::MatrixXf::Zero(quadrotor.m,1);
							}
						...
						// eig_X.block(2,0,1,mpc_params.T) = Eigen::MatrixXf::Ones(1,mpc_params.T)*current_position(2);
						badTraj = 1;
						numFailures = 0;
						firstSuccess = true;
						ii = mpc_params.nu_X;
			...
			if (mpc_params.nu_sk < mpc_params.nu_X)
			{
				for (int i = mpc_params.nu_sk; i < mpc_params.nu_X; i++)
				{
					for (unsigned int jj = 0; jj < mpc_params.T; jj++)
					{
						...
						full_trajectory[i].block(0,jj,2,1) = full_trajectory[mpc_params.nu_sk-1].block(0,mpc_params.T-1,2,1);

					}

					full_trajectory[i].block(2,0,2,mpc_params.T) = Eigen::MatrixXf::Zero(2,mpc_params.T);
					full_policy[i].block(0,0,quadrotor.m,mpc_params.T) = Eigen::MatrixXf::Zero(quadrotor.m,mpc_params.T);
					u_k_nuX[i].block(0,0,quadrotor.m,mpc_params.T) = Eigen::MatrixXf::Zero(quadrotor.m,mpc_params.T);
			...
			// cout << "Plan #: " << numTrajectoryPlans << " Pos: " << current_position.transpose() << endl;
			// for (int i = 0; i < mpc_params.nu_X; i++)
			// {
				...
				for (int ll = 0; ll < mpc_params.nu_X; ll++)
				{
					...
					for (int kk = 0; kk < mpc_params.T; kk++)
					{
						...
						for (int mm = 0; mm < quadrotor.m-1; mm++)
						{
							...
							log_f_mpc_policy_data.writeToLog(to_string(u_k_nuX[ll](mm,kk)) + ",",0);

						}
						log_f_mpc_policy_data.writeToLog(to_string(u_k_nuX[ll](quadrotor.m-1,kk)) + ",",0);
						...
						log_f_mpc_policy_data.writeToLog(to_string(v_k_nuX[ll](quadrotor.m-1,kk)),1);
...
void F_MPC_UNCUT::objective_function_value(Eigen::MatrixXf* X, Eigen::MatrixXf* U, int segment_number)
{
	...
	goal_n(0,0) = goal(0,0); // x
	goal_n(1,0) = goal(1,0); // y
	...
	for (int i = 0; i < mpc_params.T-1; i++)
	{
		...
		tildeq[i].block(0,0,quadrotor.n,1) = mpc_params.eig_q_rk[i];
